...
int main()
{
    ...
    StateCreate(&sta[0],NULL,task1,NULL,2,NEXT_STATE_MISS_COM(2,&sta2),NEXT_STATE_MISS_COM(3,&sta3));//创建状态1：无状态开始任务，状态执行任务为task1，无状态结束任务，下一状态数量2 ； NEXT_STATE_MISS_COM(2,&sta2)：2为其中一个切换下一状态事件，sta2其中一个下一状态结构体（下面同理）
    StateCreate(&sta[1],NULL,task2,NULL,2,NEXT_STATE_MISS_COM(1,&sta1),NEXT_STATE_MISS_COM(3,&sta3));
