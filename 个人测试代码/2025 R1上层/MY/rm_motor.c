...
void RM_Init(u8 CANx,RM_RX_DATA*RM_Rx_Buf,u8 Buf_Len)//RM电机CAN初始化
{
    ...
    RM_Rx_Data=RM_Rx_Buf;//得到电机数据缓存区
...
void RM_Tx(uint32_t  id,int current1,int current2,int current3,int current4)//发送四路电机电流值
{
    ...
    Can_Tx(RM_CANx,CAN_ID_STANDARD,id,data,8,0);//启动CAN发送（使用标准ID，ID为RM_ID，数据为八字节data，数据长度为8，不发送换行）
}
...
{
    u8 i;
    for(i=0;i<RM_Rx_Len;i++)//RM_Rx_Len在初始化设定，用
    {
