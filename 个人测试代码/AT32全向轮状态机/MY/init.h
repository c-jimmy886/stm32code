...
typedef void (*UART_SendByte_Func)(uint8_t data);
...
void Uart_Printf(u8 USARTx, char *format, ...);// 可重用的printf函数，支持多个串口和多种数据类型
void Uart_PrintfLn(u8 USARTx, char *format, ...);// 带自动换行的printf函数
void Uart_SendHex(u8 USARTx, uint8_t *data, uint16_t len);// 发送十六进制数据
void Uart_PrintHex(u8 USARTx, char *description, uint8_t *data, uint16_t len);// 发送带描述的十六进制数据
void Uart_SendNumber(u8 USARTx, int32_t number);// 发送数字（整数）
void Uart_SendFloat(u8 USARTx, float number, uint8_t decimal_places);// 发送浮点数
//
