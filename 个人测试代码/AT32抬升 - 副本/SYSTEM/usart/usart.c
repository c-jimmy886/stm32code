...
u8 DMA_RX1=1;
extern int dma_data_rx_p1;
extern int dma_data_rx_len1;
void USART1_IRQHandler(void)                	//串口1中断服务程序
{
	u8 Res;
    if(usart_flag_get(USART1, USART_IDLEF_FLAG) != RESET)//空闲中断
    {
        DMA_RX1=1;//数据接收完成，标记为空闲
        if(dma_data_rx_p1>=0)
        {
            dma_data_rx_p1+=dma_data_rx_len1;
            if(dma_data_rx_p1>=DMA_BUF_SIZE)dma_data_rx_p1-=DMA_BUF_SIZE;
            dma_data_rx_len1=DMA_BUF_SIZE-dma_data_number_get(DMA2_CHANNEL1)-dma_data_rx_p1;
            if(dma_data_rx_len1<=0)dma_data_rx_len1+=DMA_BUF_SIZE;
        }
        usart_flag_clear(USART1, USART_IDLEF_FLAG);
    }
	if(usart_flag_get(USART1, USART_RDBF_FLAG) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	{
