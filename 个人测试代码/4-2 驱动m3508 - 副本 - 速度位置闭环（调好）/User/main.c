...
uint16_t move_flag=0;//当按下按键，置1，然后小步增加
uint8_t move_time=0;//小步增加次数
...
int main(void)
{
    ...
    PID_Init(&speed_pid,8.0f,0.1089f,1.0f,1.6f,0.007f,2.4f,20000,-20000);//位置环调好了，速度环还不行
...
//			motor.target_position-=2000;
			keynum=0;
			move_flag=1;
			move_time=0;
		}
		if(move_flag==1&&move_time<=10)
		{
