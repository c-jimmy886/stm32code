...
/*达妙使用指南：首先要发送使能位，使用DM_Motor_Cmd(u8 id,u8 cmd)
               然后在main顶上创建DM_RX_DATA CAN1_DM_BUF,这个数组传如init函数里面
			   调用函数DM_Motor_Init(u8 CANx,DM_RX_DATA*DM_Rx_Buf,u8 Buf_Len,uint16_t mode)
			   然后注意init函数里面不同的can使用的中断函数，
			   中断回调函数还要在main里面CAN1_RX0_IRQ=CAN1_IRQ;//装载CAN1回调函数  dm
			   然后可以在中断回调函数里面调用dm_rx接收，然后dm_rx接收到的数据地址和DM_Rx_Buf的地址一样，数据会存到里面
			   然后，可以直接在main函数里面调用DM_Rx_Buf的数据
...
//达妙电机接收id对应master id，发送数据对应can_id,速度位置要加上偏移
//达妙电机初始化
